...
function main() {
    ...
    // 4. Find Search Results (from hybrid_search tool calls)
    const searchFiles = files.filter(f => f.includes('scientist') && f.includes('call') && f.endsWith('.json'));
    ...
    for (const f of searchFiles) {
        try {
            ...
            // Check if it's a hybrid_search function call response
            if (json.functionCall && json.functionCall.name === 'hybrid_search') {
                // Look for the response in related files
                const responseName = f.replace('call', 'response');
                if (files.includes(responseName)) {
                    const response = JSON.parse(fs.readFileSync(path.join(artifactsDir, responseName), 'utf-8'));
                    fs.writeFileSync(path.join(artifactsDir, 'search_results.json'), JSON.stringify(response, null, 2));
                    console.log(`✅ Normalized Search Results from ${responseName}`);
    ...
    if (!foundSearchResults) {
        console.warn('⚠️ No Search Results found. Writing empty file.');
        fs.writeFileSync(path.join(artifactsDir, 'search_results.json'), '{}');
    }
}
