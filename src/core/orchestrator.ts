...
import { Agent, Task, OrchestratorConfig, ConfigFile, AgentMode } from '../types/index.js';
...
`;
  ...
  private getSparcModePrompts(): Record<string, string> {
    return {
      ...
      // Kilig Specific Roles
      coordinator: `You are the Kilig Root Agent. Your goal is to orchestrate the video generation pipeline.`, 
      ingestor: `You are an ingestor agent. Find and retrieve relevant scientific papers.`, 
      analyzer: `You are a scientific analyzer. Extract core concepts, methodology, and results.`, 
      narrative: `You are a narrative architect. Create educational video scripts from scientific analysis.`, 
      designer: `You are a scene graph designer. Generate Motion Canvas JSON from scripts.`, 
      validator: `You are a QC validator. Ensure JSON schema compliance and design system integrity.`, 
