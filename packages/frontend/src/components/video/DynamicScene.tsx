...
export default makeScene2D(function* (view) {
  // Access the sceneGraph passed via inputProps
  // @ts-ignore
  const sceneGraph = useScene().variables.get('sceneGraph')();
  ...
  for (const scene of sceneGraph.scenes) {
    ...
    // Execute collected animations in parallel or sequence?
    // The schema doesn't specify timeline, so we'll assume they start with the scene or are sequential?
    // Let's assume they run in parallel for now to be safe, or just wait for duration.
    // Ideally the schema would have 'startTime'.
